...
class plot:
    ...
    def single(self, type, save=True, close=False, **kwargs):
        dir_path = os.path.join(self.plots_path, 'single')
    ...
    def compare(self, type, save=True, close=False, **kwargs):
        dir_path = os.path.join(self.plots_path, 'compare')
        if not os.path.exists(dir_path):
            os.mkdir(dir_path)
...
class movie:
    def __init__(self, astra, animations_path, axes, num_bins, dist, name) -> None:
        self.astra = astra
        ...
        self.axs.set_title(f"|B|={self.astra.input['solenoid']['maxb(1)']} T, q={self.dist.input['total_charge']['value']} nC, Mean z={round(mean(beam.z), 3)} m")
